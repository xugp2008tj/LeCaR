...
def get_algo_name(param):
    ...
    if param['algorithm'].lower() == "lecar3":
            algo_name= "LeCaR"
    else:
            algo_name= "ARC"
    return algo_name
...
def run(param, ax_weight, ax_hitrate, exp_cnt):
    ...
    if visualize :
        ...
        ax_hitrate.plot(range(len(hitrate)), hitrate,label=algo_name+ " - " + str(hitrate_plot), color =colors[exp_cnt%2] ,alpha=0.8)
