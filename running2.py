...
def run_experiment(keys, values, exp_num = 1):
    ax_weight = plt.subplot(3,1,1)
    ax_hoarding = plt.subplot(3,1,2)
    ax_hitrate = plt.subplot(3,1,3)
    ...
    for vals in itertools.product(*tuple(values)):
        ...
        hit_rate, duration = run(param, ax_weight, ax_hoarding, ax_hitrate)
    ...
    plt.savefig("output/%s_%s_%s_%d.png" % ( sys.argv[config_idx], param['experiment_name'],param['algorithm'], exp_num))
